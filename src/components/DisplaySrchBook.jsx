...
import { Button, Card, CardActions, CardContent, CardHeader, Grid, Rating } from "@mui/material";
    ...
    return<>
        <Grid container spacing={2} p={7}>
            {book.map((item)=>{
                ...
                return<>
                <Grid item lg={3} md={4} sm={6} xs={12} key={id} p={3} >
                    <Card variant="outlined" style={{backgroundColor:"lightblue"}}>
                        {/* <CardMedia src={thumbNail} style={{height: 0, paddingTop: '56%' }}/> */}
                        <Grid item pl={9} pt={3}>
                        <img src={thumbNail} alt="[no cover]" width='60%' height='auto' />
                        </Grid>
                        <CardHeader title={tiTle} subheader={autHor[0]} />
                        <CardContent>
                            <Rating value={avgRating} readOnly precision={0.5}/><hr/>
                            <div>
                            <ul>
                                <li>{subTitle}</li>{subTitle?<hr/>:''}
                                <li>Pages: {paGes}</li><hr/>
                                <li>Publisher: {pubLisher}</li><hr/>
                            </ul>
                            </div>
                        </CardContent>
                        <CardActions>
                            <Button size="small" onClick={()=>{setShow(true);setItem(item)}}>Learn More</Button>
                        </CardActions>                    
                    </Card>
                </Grid>
